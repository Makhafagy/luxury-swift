...
const Carousel = () => {
    ...
    const handlePrev = () => {
        setActiveItem(activeItem === 0 ? images.length - 1 : activeItem - 1);
    };
        ...
        <div id="controls-carousel" className="relative w-full" data-carousel="static">
            <div className="relative h-56 overflow-hidden rounded-lg md:h-96">
                {images.map((image, index) => (
                    <div key={index} className={`hidden duration-700 ease-in-out ${activeItem === index ? 'block' : ''}`} data-carousel-item={activeItem === index ? 'active' : ''}>
                        <img src={image} className="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt={`Carousel ${index + 1}`} />
                    </div>
                ))}
            </div>
